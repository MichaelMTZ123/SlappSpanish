...
export const DailyQuests = ({ quests }: { quests: DailyQuest[] }) => {
    ...
    const getLocalizedDescription = (quest: DailyQuest) => {
        // Map IDs to translation keys
        if (quest.id === 'q1') return t('q1_desc');
        if (quest.id === 'q2') return t('q2_desc');
        if (quest.id === 'q3') return t('q3_desc');
        return quest.description; // Fallback
    };
                ...
                {quests.map(quest => (
                    <div key={quest.id} className="flex items-center justify-between p-3 bg-white/50 dark:bg-gray-800/50 rounded-xl shadow-sm">
                        <div className="flex items-center gap-3">
                             ...
                             <div>
                                 <p className={`text-sm font-semibold ${quest.completed ? 'text-gray-500 line-through' : 'text-gray-800 dark:text-gray-200'}`}>
                                    {getLocalizedDescription(quest)}
                                 </p>
